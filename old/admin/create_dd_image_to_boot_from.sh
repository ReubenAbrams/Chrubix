#!/bin/bash
#
# create_dd_image_to_boot_from.sh
# - generates an ArchLinux SD card or thumb drive from within ChromeOS
# - Hugo then uploads the tarball to the Internet (Dropbox)
# - stage1.sh uses it
# - if called with CLI params, it re-works the kernel & boot loader
#
#
#
# To run, type:-
# cd && wget bit.ly/1hq8O7s -O xx && sudo bash xx
#################################################################################
...
# That tarball will be uploaded to Dropbox, whence it will be downloaded & used by stage1.sh :)
	...
	echo "Successfully installed filesystem from $fsurl"
	...
	setup_rootpassword		 $root	|| failed "setup_rootpassword() failed"
	move_all_data_from_p3_to_p2 $root $dev $dev_p $petname "$dev_p"3 $CRYPTOROOTDEV /tmp/dest_mount # ... and leave dest mounted
...
# FIXME Is this necessary? YES. These tools are started (not enabled) by post-login script. They are NOT enabled at boot-up. They are STARTED automatically by the postlogin script.
	for f in privoxy tor freenet; do # i2p
		chroot_this $root "systemctl disable $f" || failed "Unable to disable $f"
	done
}
...
set -e
if [ "$1" = "redo_kernel" ] ; then
	[ -e "/etc/.nopheasants" ] && NOPHEASANTS="nopheasants" || NOPHEASANTS=""
	[ -e "/etc/.nokthx" ] && NOKTHX="nokthx" || NOKTHX=""
# We're not ready to redo magic numbers. Sorry.
	bn="`basename $CRYPTOROOTDEV`"
	dev=`cat /proc/cmdline | tr '=' '\n' | grep "$bn" | grep -v mapper | head -n1 | cut -d':' -f1`
	if [ "$dev" = "" ] ; then
...
#echo "serialno=$serialno"
	haystack="`deduce_whitelist $dev`"
	[ "$2" = "flip_pheasants" ] && tweak_sources_according_to_noktxh_and_nopheasants_variables
	mount | grep /dev/mapper/encstateful &> /dev/null && myroot=`mount | grep "3 on /tmp/" | head -n1 | cut -d' ' -f3` || myroot=/
	echo "myroot=$myroot dev_p=$dev_p"
	redo_mbr $myroot $dev_p || failed "Failed to redo mbr"
	exit $?
fi

failed "THIS SCRIPT HAS BEEN REPLACED. Use create_archlinux_tarball.sh instead!"
